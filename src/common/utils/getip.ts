...
export const getIp = function (ctx: Context) {
  ...
  if (ipStr && !Array.isArray(ipStr)) {
    const ipArray: string[] = ipStr.split(',');
  ...
  if (ipStr && !Array.isArray(ipStr)) {
    ...
    if (ipArray.length > 1) {
      ...
      for (let i = 0; i < ipArray.length; i++) {
        const ipNumArray: string[] = ipArray[i].split('.');

        const tmp: string = ipNumArray[0] + '.' + ipNumArray[1];
...
export const getClientIp = function (
  ctx: Context | koa.ParameterizedContext<koa.DefaultState, koa.DefaultContext>,
) {
  const { req } = ctx;
  let ip =
